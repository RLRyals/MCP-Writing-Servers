-- =============================================
-- Migration 024: Update NPE Schema to Full Specification
-- =============================================
-- This migration updates the NPE tables to match the complete
-- Narrative Physics Engine specification.
--
-- Changes:
-- - Drops simplified NPE tables from migration 023
-- - Creates comprehensive NPE schema with 9 tables
-- - Adds proper causality tracking, decision validation, and compliance scoring
--
...
-- =============================================
-- STEP 1: Drop old NPE tables from migration 023
-- =============================================

DROP TABLE IF EXISTS npe_pov_state CASCADE;
DROP TABLE IF EXISTS npe_scene_validation CASCADE;
DROP TABLE IF EXISTS npe_character_decisions CASCADE;
DROP TABLE IF EXISTS npe_causal_links CASCADE;
DROP TABLE IF EXISTS npe_causality_chains CASCADE;

RAISE NOTICE 'Dropped old NPE tables from migration 023';

-- =============================================
-- STEP 2: Create comprehensive NPE schema
-- =============================================

-- Table 1: Causality Chain Tracking
CREATE TABLE npe_causality_chains (
    id TEXT PRIMARY KEY,
...
CREATE TABLE npe_pacing_analysis (
    id TEXT PRIMARY KEY,
    book_id INTEGER NOT NULL REFERENCES books(id) ON DELETE CASCADE,
    chapter_id INTEGER REFERENCES chapters(id) ON DELETE CASCADE,
...
CREATE TABLE npe_stakes_pressure (
    id TEXT PRIMARY KEY,
    book_id INTEGER NOT NULL REFERENCES books(id) ON DELETE CASCADE,
    chapter_id INTEGER NOT NULL REFERENCES chapters(id) ON DELETE CASCADE,
    scene_id INTEGER REFERENCES chapter_scenes(id) ON DELETE CASCADE,
...
CREATE TABLE npe_information_economy (
    id TEXT PRIMARY KEY,
    book_id INTEGER NOT NULL REFERENCES books(id) ON DELETE CASCADE,
...
CREATE TABLE npe_relationship_tension (
    id TEXT PRIMARY KEY,
...
CREATE TABLE npe_compliance_summary (
    id TEXT PRIMARY KEY,
    book_id INTEGER NOT NULL REFERENCES books(id) ON DELETE CASCADE,
    chapter_id INTEGER REFERENCES chapters(id) ON DELETE CASCADE,
...
RAISE NOTICE 'Created 9 NPE tables with comprehensive narrative physics tracking.';
END $$;
