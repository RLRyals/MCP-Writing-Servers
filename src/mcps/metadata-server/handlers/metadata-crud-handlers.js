// src/mcps/metadata-server/handlers/metadata-crud-handlers.js
// Generic metadata CRUD handlers, shared by any server that wires them in
// (metadata-server stdio transport, book-planning-server HTTP transport).
// Response shape (prose text, not JSON) must stay byte-identical across transports.

import { metadataCrudToolsSchema } from '../schemas/metadata-crud-schemas.js';

export class MetadataCrudHandlers {
    constructor(db) {
        this.db = db;
    }

    getMetadataCrudTools() {
        return metadataCrudToolsSchema;
    }
